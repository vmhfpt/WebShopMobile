...
z-index : 1000 ;" class="alert alert-success"> 
    ...
    <div class="search-product">
       <form method="GET" action="">
           <center><h5> Tìm Kiếm Đơn Hàng</h5></center>
           <input value="<?= $search_id?>" name="search_id" type="number" class="form-control" placeholder="Enter Your Key Search Id Order" id="id_seach">
           <input value="<?=$_SESSION['Search_name']?>" name="search_name" type="text" class="form-control" placeholder="Enter Your Key Search Name Order" id="name_searche">
  ...
  <table class="table table-bordered">
    ...
    <tbody>
     ...
     if ($Search == false && $_SESSION['Search_name'] == null){
           $sql = 'SELECT * FROM oder LIMIT '. $limit .' OFFSET '. $index .'';
     } else {
         if(isset($_GET['search_name']) && $_GET['search_name'] != '' || $_SESSION['Search_name'] != null) {
              $sql = 'SELECT * FROM oder WHERE name like "%'.$_SESSION['Search_name'].'%" LIMIT '. $limit .' OFFSET '. $index .'';
         }
         if(isset($_GET['search_id']) && $_GET['search_id'] != '' && $_SESSION['Search_name'] == null)  {
             $sql = 'SELECT * FROM oder WHERE id = ' . $_GET['search_id'] ;
         }
     }
     ...
     <?php }}else {
       ...
       <h6 style="float: right;
         margin-right : 12px "> Có tất cả <i><?=count($data)?></i> đơn hàng trên <i><?=ceil(count($data)/$limit)?></i> trang</h6>
    </tbody>
  </table>
  <ul class="pagination">
<?php
