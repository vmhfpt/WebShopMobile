    ...
    if(isset($_POST['optradio'])){
      //  var_dump($_POST['optradio']);die();
        foreach ($_POST['optradio'] as $key){
           ...
           if($boolened == true){
            $sql_add = 'INSERT INTO `products_bought` (`id`, `id_product`) VALUES (NULL, '.$key.')';
            execute($sql_add);
           }
        } 
    header('location:product_edit.php?action=1');
    }
...
<?php
   ...
   if (($current_page - 2) > 0 ){
      echo '<li  class="page-item"><a  class="page-link" href="product_add.php?page='.($current_page - 1).'">Previous</a></li>';
   }
  ...
  ?>"><a  class="page-link" href="product_add.php?page=<?=$i?>"><?=$i?></a></li> 
    <?php } ?>
    <?php
    if (($current_page + 2) <= ceil(count($data)/$limit)){
        echo ' <li  class="page-item"><a  class="page-link" href="product_add.php?page='.($current_page + 1).'">Next</a></li>';
     }
?>
</ul>
    </div>
